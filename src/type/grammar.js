...
export function algoritmoAnalisis(declaration) {
  ...
  while (stack.length > 0 && tokenIndex < tokens.length) {
    console.log("Iteración:", tokenIndex);
    console.log("Stack:", stack);
    console.log("Tokens procesados:", tokens.slice(0, tokenIndex));
    const X = stack.pop();
    ...
    if (esCategoria(X, currentToken)) {
      ...
      if (production.length > 0) {
        ...
        console.log(`Error: No hay producción para ${X} con ${currentToken}`);
        return { result: false, stackContent: stackContent.concat(`Error: No hay producción para ${X} con ${currentToken}`) };
      }
    } else {
      console.log(`Error: Se esperaba ${X}, se encontró ${currentToken}`);
...
function obtenerProduccion(noTerminal, simboloEntrada) {
  console.log("Obteniendo producción para:", noTerminal, simboloEntrada);
...
/*function validateVariableDeclaration(input) {
  ...
  console.log("Cadena:", result ? "Válida" : "No válida");
